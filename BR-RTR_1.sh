...
iptables -t nat -A PREROUTING -i $int_type -p tcp -m multiport --dports 8080,2026 -j DNAT --to-destination 192.168.1.1
if [[ $Hypervisor == 2 ]]; then
  mkdir -p /etc/net/ifaces/enp7s2
  echo -e "BOOTPROTO=static\nTYPE=eth" > /etc/net/ifaces/enp7s2/options
  echo "192.168.1.1" > /etc/net/ifaces/enp7s2/ipv4address
else
  mkdir -p /etc/net/ifaces/ens37
  echo -e "BOOTPROTO=static\nTYPE=eth" > /etc/net/ifaces/ens37/options
  echo "192.168.1.1" > /etc/net/ifaces/ens37/ipv4address
fi
...
TYPE=iptun
TUNTYPE=gre
TUNLOCAL=172.16.80.1
TUNREMOTE=172.16.70.1
TUNOPTIONS='ttl 64'
EOF
